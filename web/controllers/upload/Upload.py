...
def uploadPic():
    ...
    callback_target = 'window.parent.upload' # 调用父页面的方法
    if upfile is None:
...
def listImage():
    ...
    list = query.order_by(Image.id.desc()).limit(page_size).all() # 取id<start的倒序page_size个值
    images = []
