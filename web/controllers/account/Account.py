...
def index():
    ...
    if 'mix_kw' in req:  # 混合查询
        rule = or_(User.nickname.ilike("%{0}%".format(req['mix_kw'])), User.mobile.ilike("%{0}%".format(req['mix_kw'])))
        query = query.filter(rule)

    if 'status' in req and int(req['status']) > -1: # 状态查询
        query = query.filter(User.status == int(req['status']))

    page_params = {
        'total': query.count(), # 总共记录数
        'page_size': app.config['PAGE_SIZE'], # 每页大小
        'page': page, # 当前页数
...
def info():
    ...
    offset = (page - 1) * app.config['PAGE_SIZE']
    limit = page * app.config['PAGE_SIZE']
    list = query.order_by(AppAccessLog.uid.asc()).all()[offset: limit]
